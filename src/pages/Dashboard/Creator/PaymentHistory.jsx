  ...
  const fetchPayments = async () => {
    try {
      const response = await fetch(
        `http://localhost:5000/withdrawals/${user.email}`
      );
  ...
  return (
    ...
    >
      ...
      <p className="mt-3 max-w-2xl text-sm leading-7 text-[#667085] sm:text-base">
        View all your withdrawal requests and payment status.
      </p>
      <motion.div
  initial={{ opacity: 0, y: 20 }}
  animate={{ opacity: 1, y: 0 }}
  transition={{ duration: 0.5, delay: 0.2 }}
  className="mt-8 overflow-hidden rounded-3xl border border-[#E9E7E1] bg-white shadow-[0_12px_35px_rgba(16,24,40,0.05)]"
>
        ...
        {payments.map((payment) => (
          ...
          >
            <td className="px-6 py-4">
              {new Date(payment.withdraw_date).toLocaleDateString()}
            </td>
